...
export class AudioSystem {
  ...
  playSound(type, options = {}) {
    ...
    switch (type) {
      // ── Gunshot ─────────────────────────────────────────────────
      case "shoot": {
        const sequence = weapon?.audio?.shoot ??
          weapon?.audio?.default ?? [
            {
              action: "noiseBurst",
              freq: 55,
              q: 0.5,
  ...
  unmute() {
    ...
    this.musicGain.gain.value =
      this._previousMusicVolume ?? GameConfig.AUDIO.MUSIC_VOLUME;
  }
